...
from matlab_desktop_proxy.default_configuration import config
...
class InstallNpm(install):
    def run(self):
        ...
        target_dir = Path(self.build_lib) / matlab_desktop_proxy.__name__ / "gui"
...
setuptools.setup(
    ...
    entry_points={
        matlab_desktop_proxy.get_entrypoint_name(): [
            f"{matlab_desktop_proxy.get_default_config_name()} = matlab_desktop_proxy.default_configuration:config"
        ],
        "console_scripts": [
            f"{matlab_desktop_proxy.get_executable_name()} = matlab_desktop_proxy.app:main"
        ],
    },
